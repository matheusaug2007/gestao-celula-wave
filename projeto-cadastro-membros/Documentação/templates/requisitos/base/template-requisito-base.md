...
- Utilize o bloco "{{NOME_DA_SECAO_INICIAL}}" para descrever a funcionalidade principal.
- Repita o bloco "{{NOME_DA_SUBFUNCAO_OU_PROCESSO}}" para subfluxos, exceções ou processos auxiliares.
- Se uma subseção não se aplicar, remova o bloco correspondente (não deixe placeholders).
...
- Não use listas de campos isoladas (isso deve ficar nas subseções específicas ou em regras).
- Não duplicar conteúdo já coberto em Contextualização ou Referências.
...
{{DESCRICAO_DA_TELA_OU_ENTRADA}}

<!--
AGENT IA - INSTRUÇÕES DE PREENCHIMENTO:
Descreva a composição da tela: filtros + listagem paginada. Se houver imagem do protótipo, inserir abaixo.
-->

{{DESCRICAO_TELA}}

![{{DESCRICAO_DA_IMAGEM}}]({{CAMINHO_DA_IMAGEM}})



## {{NOME_DA_SUBFUNCAO_OU_PROCESSO}}

<!--
AGENT IA - INSTRUÇÕES DE PREENCHIMENTO:
Descreva o comportamento do sistema, não apenas o passo a passo do usuário.
Foque em regras, validações e decisões do sistema.
-->
...
## Cenário 1: {{TITULO_FLUXO_PRINCIPAL}}

**Dado que** {{CONTEXTO_PRE_CONDICAO}}
...
**Então** o sistema deve:
  ...
  - {{RESULTADO_ESPERADO_N}}

---

## Cenário 2: {{TITULO_VALIDACAO_OU_REGRA}}
...
## Cenário N: {{TITULO_CENARIO_ADICIONAL}}
